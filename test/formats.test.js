...
// --- template ---
const html = await compileTemplate('assin.html', Buffer.from('<b>{{nome}}</b>'));
assert.strictEqual(html.format, 'html');
assert.deepStrictEqual(extractPlaceholders(html.html), ['nome']);

const mjml = await compileTemplate('assin.mjml', Buffer.from('<mjml><mj-body><mj-text>{{cargo}}</mj-text></mj-body></mjml>'));
assert.strictEqual(mjml.format, 'mjml');
assert.ok(mjml.html.includes('<'), 'mjml compila para html');

// sem extensão: detecta pelo conteúdo
const mjmlSniff = await compileTemplate('', Buffer.from('<mjml><mj-body><mj-text>{{x}}</mj-text></mj-body></mjml>'));
assert.strictEqual(mjmlSniff.format, 'mjml');
assert.strictEqual((await compileTemplate('', Buffer.from('<div>oi</div>'))).format, 'html');

// formato não suportado falha alto e nomeia os suportados
await assert.rejects(
  () => compileTemplate('assin.pptx', Buffer.from('PK\x03\x04binário')),
  /não suportado.*pptx.*mjml/s,
  'pptx deve ser rejeitado com mensagem clara'
);

// docx agora é suportado, mas um zip inválido falha alto
await assert.rejects(() => compileTemplate('assin.docx', Buffer.from('não é um zip')));
